...
export_dir = "./models/alexnet/" + str(int(time.time()))
...
with tf.Session() as sess:
    ...
    allimgs = np.zeros(shape=(4,227,227,3), dtype=np.float)
    ...
    SignatureDef = tf.saved_model.signature_def_utils.predict_signature_def(inputs={"input": x}, outputs={"softmax": softmax})

    builder.add_meta_graph_and_variables(sess, tags=[tag_constants.SERVING], signature_def_map={tf.saved_model.signature_constants.CLASSIFY_INPUTS:SignatureDef})
